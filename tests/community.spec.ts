...
import { 
    ...
    navigateToTabPage, 
    checkIfUserLoggedInTheUI,
    clickAndNavigateWithLink,
...
test("test navigating to community", async ({ page }) => {
    await navigateToTabPage(page, 'communities');

    await testLoadingCardsBasedOnEntity(
        page,
        'communities',
        'community'
    );
...
test("test creating a community, and refresh to see if it exists.", async ({ page }) => {
    test.setTimeout(150_000)
    await navigateToSpecificPage(page, '/communities');

    await openModalAfterClickButton(page, 'createcommunitybutton');
